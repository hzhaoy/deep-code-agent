...
def test_chat_log_upserts_single_todos_card():
    ...
    async def run_test():
        async with App().run_test() as pilot:
            ...
            first = chat_log.upsert_todos_card([{"content": "Plan", "status": "pending"}])
            second = chat_log.upsert_todos_card([{"content": "Verify", "status": "completed"}])

            assert first is second
            assert len([child for child in chat_log.children if isinstance(child, TodosProgressCard)]) == 1
...
def test_chat_log_keeps_todos_card_pinned_after_new_messages():
    ...
    async def run_test():
        async with App().run_test() as pilot:
            ...
            card = chat_log.upsert_todos_card([{"content": "Plan", "status": "pending"}])
            ...
            moved = chat_log.upsert_todos_card([{"content": "Plan", "status": "completed"}])
